...
router.post("/upload", authMiddleware, async (req, res) => {
    ...
    video.mv(videoPath, async (err) => {
        ...
        const rec = {
            "qut-username": req.user?.sub || process.env.QUT_USERNAME,
            "createdAt": new Date().toISOString(),
        ...
        try {
            ...
            res.json({ message: "Video uploaded successfully", fileId: rec.id, filePath: videoPath });
        } catch (e) {
            console.error(e);
...
router.get("/mine", authMiddleware, async (req, res) => {
    ...
    // 游標：用 LastEvaluatedKey 直接傳回來的物件最準確
    let cursor = undefined;
    if (req.query.cursor) {
      try { cursor = JSON.parse(Buffer.from(req.query.cursor, "base64").toString("utf8")); } catch {}
    }
  
    const user = req.user?.sub || process.env.QUT_USERNAME;
...
router.get("/:id/download", authMiddleware, async (req, res) => {
    ...
    const isOwner = rec["qut-username"] === (req.user?.sub || process.env.QUT_USERNAME);
    const isAdmin = req.user?.role === "admin";
    if (!isOwner && !isAdmin) return res.status(403).send("Forbidden");

    const p = rec.outputPath || rec.inputPath;
    if (!p || !fs.existsSync(p)) return res.status(404).send("File missing");

    const suggested = rec.outputPath ? rec.origName.replace(/\.[^.]+$/, ".gif") : rec.origName;
    res.setHeader("Content-Disposition", `attachment; filename="${suggested}"`);
    return res.download(p, path.basename(p));
});
...
router.patch("/:id/tags", authMiddleware, async (req, res) => {
    ...
    const updated = await updateTags(req.params.id, tags);
    return res.json({ message: "Tags updated", file: updated });
});
